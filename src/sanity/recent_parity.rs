...
use log::info;
use redgold_schema::{EasyJson, error_info, RgResult, WithMetadataHashable};
use redgold_schema::observability::errors::EnhanceErrorInfo;
use redgold_schema::structs::{Hash, TransactionEntry, UtxoId};
...
impl RecentParityCheck {
    ...
    async fn validate_transaction_window(&self, recent_tx: &Vec<TransactionEntry>) -> RgResult<()> {
        ...
        for txe in recent_tx {
            if let Some(tx) = txe.transaction.clone() {
                let hash = tx.hash_or();
