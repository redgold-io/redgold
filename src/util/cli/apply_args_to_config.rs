...
pub fn apply_args_initial(rg_args: Box<RgArgs>, config: Box<ConfigData>) -> Box<ConfigData> {
    ...
    if let Some(sdp) = rg_args.config_paths.secure_data_path.as_ref() {
        sd.path = Some(sdp.clone());
    }
    if let Some(smh) = rg_args.config_paths.secure_data_config_path.as_ref() {
    ...
    if rg_args.config_paths.data_path.is_some() {
        config.data = rg_args.config_paths.data_path;
    }
