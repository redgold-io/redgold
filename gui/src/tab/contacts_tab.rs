...
impl AddressTabState {
    ...
    pub fn saved_address(&self) -> SavedAddress {
        ...
        SavedAddress {
            ...
            contact: self.contact_name.clone(),
        ...
        G: GuiDepends + Clone + 'static + Send
    {
        ...
        self.new_address_input.address_box_label = "Address:".to_string();
        self.new_address_input.show_valid = true;
        ...
        let saved = config.local.unwrap_or_default().saved_addresses.unwrap_or_default();

        let is_debug_round = self.debug.clone();
        ...
        if self.new_address_input.valid {
            if ui.button("Add & Save").clicked() {
                ...
                self.debug = true;
                self.new_address = "".to_string();
                self.new_address_input.reset();
                self.contact_name = "".to_string();
                self.address_name = "".to_string();
                self.related_keysource_input = "".to_string();
                self.show_new_address = false;
            }
        }
        ...
        if saved.len() > 0 {
            ...
            //
            // if is_debug_round {
            //     println!("okay saved gt 1 lets show data {:?}", data.clone())
            // }
